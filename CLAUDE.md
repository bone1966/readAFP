...
Outline fonts (Type 1 PFB / CID) have no bitmaps, so `_outline_font_page()`
renders a metadata sheet instead of leaving them blank: typeface, the
technology sniffed from the FNG program header (`%!PS-AdobeFont` ⇒ Adobe
Type 1 PFB), the character count, and a `GCGID · glyph name · increment`
grid. FNI records repeat per rotation, so `_dedup_orientations()` keeps
one entry per GCGID and reports the orientation count; `_fnn_glyph_names()`
resolves each GCGID to its readable name via the Font Name Map (FNN).
Specimen text sets `TextRun.fit = False` so the fixed grid is never
width-fitted by `render._fit()`.
...
- FOCA outline fonts (Type 1 PFB / CID, PatTech X'1F'/X'1E') — decoded to
  a metadata specimen (typeface, technology, FNI metrics, FNN glyph
  names); the embedded vendor outline shapes are not yet rasterized, so
  the font's true glyphs are not drawn. Rasterizing the Type 1 charstrings
  (the "render what a printer would mark" goal) is the next project.
...
specimens; FOCA outline-font metadata specimens (Type 1 PFB detection,
orientation dedup, FNN glyph names, fit-exempt grid); page overlays
(BMO/EMO + IPO); `__version__ = "1.0.0"` shown in the About modal with
